...
### Организация композиции компонентов
ArrowButton: для открытия и закрытия панели с настройками.
ArticleParamsForm: основной компонент, который содержит форму редактирования свойств.

### Хранение состояния
Для управления открытием и закрытием панели (ArticleParamsForm) будет использоваться состояние, которое будет указывать, открыта ли текущая панель или нет. Это состояние будет храниться в родительском компоненте, который будет управлять ArticleParamsForm.

### Передача данных между формой и страницей
Для передачи данных между формой (ArticleParamsForm) и другими частями приложения, понадобится обработчики событий.
